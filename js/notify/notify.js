...
PT.Notify.prototype = {
	init: function() {
		var self = this;
		self.getAllNotify(self.current, self.limit, self.start);
	},
	bindEvent: function() {},
	//获取所有通知信息集合
	getAllNotify: function(currentNum, limit, start) {
		var self = this;
		//模拟分页开始
		var infolist = [{
			type: 1,
		...
		$('#read-page').perfectPager({
			...
			callback: function(currentNum) {
				...
				$('#notifylist').empty(); //清空之前渲染数据
				self.getAllNotify(currentNum, self.limit, start);
			}
		}).appendTo($('#read-page'));
		//模拟分页结束
	...
	render: function(type, url, context, time) {
		...
		var $readPart = null;
		if(type == 1) { //0 未读 1已读
			$readPart = $('<div class="readed-part items"></div>');
		} else {
