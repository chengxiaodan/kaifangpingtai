...
PT.Creatapp.addInfo = function(config) {
...
PT.Creatapp.addInfo.prototype = {
	init: function() {
		var self = this;
		self.setAppInfo(self.appinfo); //设置包信息
...
//			}
	...
	save: function() {
		...
		//ajax requestData
		//保存成功跳转到应用商店列表页面
		window.location.href = "../../html/manage/appstore.html";
	},
	//提交审核
	submit: function() {
		var self = this;
		...
		self.render('step4'); //显示正在审核中的页面
		self.timedCount();
	},
	timedCount: function() {
		self = this;
		if(typeof self.t != undefined) clearTimeout(self.t);
		$('#timer').text(self.num);
		self.num--;
