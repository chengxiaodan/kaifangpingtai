...
var stepBar = {
	...
	init: function(id, option) {
		...
		this.animation = option.animation ? true : false;
		this.layout();
	...
	layout: function() {
		...
		this.bar.find(".ui-stepInfo:first-child .ui-stepName").addClass("judge-stepName-pre");
		this.bar.find(".ui-stepInfo .ui-stepName").addClass("judge-stepName-hind");
	},
	classHover: function() {
		if(this.change) {
			...
			this.bar.find(".ui-stepInfo .judge-stepSequence-cur").removeClass("judge-stepSequence-hind-change").addClass("judge-stepSequence-pre-change");
	...
	stepInfoWidthFun: function() {
		if(this.itemCount > 0) {
			...
			this.bar.find(".ui-stepLayout").css({
				"margin-left": -Math.floor(this.itemWidth / 2) + 0
			});
//			if(this.change) {
//				this._event();
//			}
	...
	percent: function() {
		...
		for(var i = 1; i <= 4; i++) {
			if(i == this.curStep) {
				this.bar.find(".ui-stepInfo:nth-child(" + this.curStep + ") .judge-stepSequence-hind").removeClass('judge-stepSequence-hind').addClass('judge-stepSequence-cur');
				this.bar.find(".ui-stepInfo:nth-child(" + this.curStep + ") .judge-stepSequence-pre").removeClass('judge-stepSequence-pre').addClass('judge-stepSequence-cur');
			}else if(i < this.curStep){
				this.bar.find(".ui-stepInfo:nth-child(" + i + ") .judge-stepName-pre").removeClass('judge-stepName-pre').addClass('judge-stepName-hind');
				this.bar.find(".ui-stepInfo:nth-child(" + i + ") .judge-stepSequence-cur").removeClass('judge-stepSequence-cur').addClass('judge-stepSequence-pre');
				this.bar.find(".ui-stepInfo:nth-child(" + i + ") .judge-stepSequence-hind").removeClass('judge-stepSequence-hind').addClass('judge-stepSequence-pre');
			}
		}
	},
	jump: function() {
		this.bar.find(".ui-stepInfo .ui-stepSequence").removeClass("judge-stepSequence-pre").addClass("judge-stepSequence-hind");
		this.bar.find(".ui-stepInfo .ui-stepName").removeClass("judge-stepName-pre").addClass("judge-stepName-hind");
		this.bar.find(".ui-stepInfo:nth-child(-n+" + this.triggerStep + ") .ui-stepSequence").removeClass("judge-stepSequence-pre").addClass("judge-stepSequence-hind");
		this.bar.find(".ui-stepInfo:nth-child(-n+" + this.triggerStep + ") .ui-stepName").removeClass("judge-stepName-pre").addClass("judge-stepName-hind");
		this.classHover();
	},
	_animate: function() {
...
//			stepSequence_size.padding = "6px 10px";
			...
			preClass2 = "judge-stepName-pre";
			hindClass2 = "judge-stepName-hind";
			easing = this.stepEasingForward;
		} else {
			stepSequence_size.backgroundColor = "#5CB85C";
			preClass = "judge-stepSequence-hind";
			hindClass = "judge-stepSequence-pre";
			preClass2 = "judge-stepName-hind";
			hindClass2 = "judge-stepName-pre";
		}

		this.bar.find(".ui-stepInfo:nth-child(" + this.curStep + ") .ui-stepSequence").removeClass(preClass).addClass(hindClass);
		this.bar.find(".ui-stepInfo:nth-child(" + this.curStep + ") .ui-stepName").removeClass(preClass2).addClass(hindClass2);
