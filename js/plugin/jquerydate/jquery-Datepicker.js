...
(function($, document, window, undefined) {
    ...
    var pluginName = 'Datepicker',
        defaults = {
            ...
            tplBackTody: function() {
                return '<div class="namespace-backTody"><a>回到今日</a></div>';
            },
            tplContent: function() {
    ...
    Plugin.prototype = {
        ...
        _init: function() {
            ...
            var wrapper = this.options.tplWrapper().replace(/namespace/g, this.namespace),
                backTody = this.options.tplBackTody().replace(/namespace/g, this.namespace),
        ...
        _initSections: function() {
            ...
            this.backTody = this.$picker.find('.' + this.namespace + '-backTody a');
        ...
        _generateDaypicker: function(index) {
            this._generateHeader(index, this._date.currentYear[index] + ' ' + LABEL[this.options.lang].months[this._date.currentMonth[index]]);
            ...
            switch (this.mode) {
                ...
                case 'range':
                    var formatedStart = this._formatDate(this._date.selectedDate[0], this.outputFormat),
                        formatedEnd = this._formatDate( this.options.endTime, this.outputFormat);
        ...
        show: function() {
            ...
            if (this.options.displayMode === 'inline') {
                ...
                if (this.showed === false) {
                    ...
                    // this.$el.focus();
                    this.backTody.on('click',function() { //今日
                    		var formatedStart = self.parseFormat(new Date(), "-");
                    		self.$el.val(formatedStart + ' ' + self.options.rangeSeparator + ' ' + formatedStart);
                    		self.options.callback(self.$el.val());
                    		self.hide();
                    		self.$el.blur();
					});
                    this.$picker.on('mousedown.' + this.flag, function(e) {
                    ...
                    switch (this.mode) {
                        ...
                        case 'range':
                            ...
                            if (judge === true) {
                                ...
                                this._date.selectedDate[0].setDate(this._date.selectedDate[0].getDay()-1);
